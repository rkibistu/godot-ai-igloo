...
# ── repo-specific config (ONE block — lift into .igloo.yml in the Phase-7 --repo refactor) ──
REPO="rkibistu/godot-ai-igloo"
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
if [ -f "$ROOT/.env" ]; then set -a; . "$ROOT/.env"; set +a; fi   # optional overrides below
GAME_SUBDIR="game"                                       # Godot project lives under <repo>/game
ADDON_SRC="$ROOT/$GAME_SUBDIR/addons/godot_ai"           # the dev's local (gitignored) addon
REVIEW_WORKTREE_DIR="${REVIEW_WORKTREE_DIR:-$(dirname "$ROOT")/$(basename "$ROOT")-review}"
GODOT_ZIP="${GODOT_ZIP:-$HOME/Downloads/Godot_v4.6.3-stable_mono_linux_x86_64.zip}"
GODOT_CACHE="$ROOT/.tools/godot"                         # where the zip is extracted (gitignored)
# GODOT_BIN may be set in .env to point at any host Godot 4.6.x mono binary directly.
# ────────────────────────────────────────────────────────────────────────────────────────
...
worktree_registered() { git -C "$ROOT" worktree list --porcelain | grep -qxF "worktree $WT"; }
...
git -C "$ROOT" fetch --quiet origin || { echo "review-setup: 'git fetch origin' failed." >&2; exit 1; }
...
WT_ADDON="$WT/$GAME_SUBDIR/addons/godot_ai"
if [ ! -d "$WT_ADDON" ]; then
  if [ -d "$ADDON_SRC" ]; then
    echo "== provision godot_ai addon into worktree =="
    mkdir -p "$WT/$GAME_SUBDIR/addons"
...
if [ "$NO_LAUNCH" = 1 ]; then
  ...
  mkdir -p "$ROOT/runs"
  nohup "$GODOT_BIN" --editor --path "$GAME_DIR" >"$ROOT/runs/review-issue-${ISSUE}.editor.log" 2>&1 &
